...
def transcribe_audio_with_deepinfra(file_path, output_dir=None, model='openai/whisper-large-v3', verbose=True):
    """
    Transcribe a single audio file using DeepInfra API.
    ...
    Args:
        ...
        model (str, optional): Model to use for transcription. Default is 'openai/whisper-large-v3'
    ...
    try:
        ...
        if not DEEPINFRA_API_KEY:
            return False, None, "DeepInfra API key not found in environment variables"
...
def main():
    parser = argparse.ArgumentParser(description='Transcribe a single audio file using DeepInfra')
    ...
    parser.add_argument('--model', default='openai/whisper-large-v3', 
                      help='Model to use for transcription (default: openai/whisper-large-v3)')
    
    args = parser.parse_args()
    
    success, output_file, error = transcribe_audio_with_deepinfra(
        args.file_path, 
        args.output_dir,
        args.model
    )
